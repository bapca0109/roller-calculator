...
- **BOM Weights populated — Bearings + Rubber Rings [2026-02-18]**: Added `BEARING_WEIGHT_KG` master map in `roller_standards.py` (6204–6310, 420204–420206) sourced from SKF/FAG datasheets — used as `weight_per_unit_kg` in auto-generated BOM for rollers. Rubber ring weight now computed from geometry (annular cylinder × rubber density 1100 kg/m³) using `ring_id`, `rubber_diameter`, `ring_width` — populates both `weight_per_unit_kg` and `total_weight_kg` in the WO BOM (both UI + PDF).
