...
# Housing Selection: pipe_dia -> [(housing_dia, [bearing_bores])] - UPDATED FROM EXCEL
HOUSING_OPTIONS = {
    60.8: [(56, [47])],
    76.1: [(72, [47, 52])],
    88.9: [(84, [47, 52, 62])],
    114.3: [(108, [47, 52, 62, 72, 80])],  # Added 80 for 6307 bearing
    127.0: [(121, [47, 52, 62, 72, 80])],
    139.7: [(133, [47, 52, 62, 72, 80, 90])],
    152.4: [(146, [47, 52, 62, 72, 80, 90])],
    159.0: [(153, [47, 52, 62, 72, 80, 90, 100])],
    165.0: [(159, [52, 62, 72, 80, 90, 100])]
}

# Housing Costs (INR per piece) - UPDATED FROM EXCEL
HOUSING_COSTS = {
    "56/47": 21,
    "72/47": 22, "72/52": 24,
    "84/47": 23, "84/52": 25, "84/62": 28,
    "108/47": 33, "108/52": 35, "108/62": 45, "108/72": 50, "108/80": 55,  # Added 108/80
    "121/47": 42, "121/52": 45, "121/62": 50, "121/72": 57, "121/80": 68,
    "133/47": 45, "133/52": 50, "133/62": 55, "133/72": 60, "133/80": 65, "133/90": 70,
    "146/47": 52, "146/52": 57, "146/62": 60, "146/72": 65, "146/80": 70, "146/90": 80,
    "153/47": 55, "153/52": 58, "153/62": 60, "153/72": 65, "153/80": 70, "153/90": 80, "153/100": 90,
...
FREIGHT_RATES_PER_KG = {
    (0, 50): 2.0,        # 0-50 km: ₹2/kg
    (50, 100): 3.0,      # 50-100 km: ₹3/kg
    (100, 200): 4.0,     # 100-200 km: ₹4/kg
    (200, 300): 4.5,     # 200-300 km: ₹4.5/kg
    (300, 400): 5.0,     # 300-400 km: ₹5/kg
    (400, 500): 5.5,     # 400-500 km: ₹5.5/kg
    (500, 600): 6.0,     # 500-600 km: ₹6/kg
    (600, 700): 6.5,     # 600-700 km: ₹6.5/kg
    (700, 800): 7.0,     # 700-800 km: ₹7/kg
    (800, 900): 7.5,     # 800-900 km: ₹7.5/kg
    (900, 1000): 8.0,    # 900-1000 km: ₹8/kg
    (1000, 1100): 8.5,   # 1000-1100 km: ₹8.5/kg
    (1100, 1200): 9.0,   # 1100-1200 km: ₹9/kg
    (1200, 1300): 9.5,   # 1200-1300 km: ₹9.5/kg
    (1300, 1400): 10.0,  # 1300-1400 km: ₹10/kg
    (1400, 1500): 10.5,  # 1400-1500 km: ₹10.5/kg
    (1500, 1600): 11.0,  # 1500-1600 km: ₹11/kg
    (1600, 1700): 11.5,  # 1600-1700 km: ₹11.5/kg
    (1700, 1800): 12.0,  # 1700-1800 km: ₹12/kg
    (1800, 1900): 12.5,  # 1800-1900 km: ₹12.5/kg
    (1900, 2000): 13.0,  # 1900-2000 km: ₹13/kg
    (2000, 9999): 13.5,  # 2000+ km: ₹13.5/kg
}

def get_distance_from_pincode(destination_pincode):
    """
    Calculate actual road distance from dispatch location (382433) to destination pincode.
    Uses geocoding to get coordinates and calculates estimated road distance.
    ...
    if not destination_pincode or len(destination_pincode) != 6:
        return 500  # Default for invalid pincode
    
    # Cache for origin coordinates (Ahmedabad - 382433)
    origin_coords = (23.0005237, 72.6723069)  # Pre-cached for performance
    
    try:
        geolocator = Nominatim(user_agent="convero_freight_calculator", timeout=10)
        
        # Geocode destination pincode
        dest_query = f"{destination_pincode}, India"
        dest_location = geolocator.geocode(dest_query)
        ...
        if dest_location:
            ...
            # Estimate road distance (typically 1.25-1.4x straight line in India)
            # Using 1.3 as average factor for Indian roads
            road_distance = straight_distance * 1.3
            
            logging.info(f"Distance calculated: {destination_pincode} -> {road_distance:.0f} km (straight: {straight_distance:.0f} km)")
            
            return round(road_distance)
        else:
            logging.warning(f"Could not geocode pincode: {destination_pincode}, using fallback")
            return _get_fallback_distance(destination_pincode)
            
    except Exception as e:
        logging.warning(f"Geocoding failed for {destination_pincode}: {str(e)}, using fallback")
        return _get_fallback_distance(destination_pincode)


def _get_fallback_distance(destination_pincode):
    """
    Fallback zone-based distance estimation when geocoding fails.
    Uses first 2 digits of pincode to estimate distance.
    """
    dest_zone = destination_pincode[:2] if destination_pincode else "00"
    
    # Approximate distances from Ahmedabad (382433) to major zones
    zone_distances = {
        # Gujarat (local)
        "36": 250, "37": 200, "38": 50, "39": 300,
        # Rajasthan
        "30": 650, "31": 450, "32": 300, "33": 600, "34": 550,
        # Maharashtra
        "40": 550, "41": 650, "42": 450, "43": 550, "44": 900,
        # Madhya Pradesh
        "45": 400, "46": 600, "47": 850, "48": 750, "49": 1000,
        # Delhi/NCR
        "11": 950, "12": 900, "13": 1100, "14": 1200, "15": 1500, "16": 1050, "17": 1200,
        # Uttar Pradesh
        "20": 850, "21": 1050, "22": 1200, "23": 1350, "24": 1000, "25": 950, "26": 1150, "27": 1100, "28": 1300,
        # Bihar & Jharkhand
        "80": 1550, "81": 1600, "82": 1650, "83": 1450, "84": 1500,
        # West Bengal
        "70": 1900, "71": 1950, "72": 2000, "73": 1700,
        # Odisha
        "75": 1800, "76": 1850, "77": 1650,
        # Andhra Pradesh & Telangana
        "50": 1100, "51": 1150, "52": 1200, "53": 1450, "54": 1300,
        # Karnataka
        "56": 1500, "57": 900, "58": 700, "59": 1200,
        # Tamil Nadu
        "60": 1750, "61": 1500, "62": 1800, "63": 1900, "64": 1650,
        # Kerala
        "67": 1600, "68": 1700, "69": 1850,
        # Goa
        "40": 750,
        # North East
        "78": 2500, "79": 2600,
    ...
    return zone_distances.get(dest_zone, 1000)
...
def calculate_raw_material_cost(pipe_dia, pipe_length_mm, shaft_dia, bearing_number, bearing_make="china", rubber_dia=None, pipe_type="B", shaft_end_type="B", custom_shaft_length=None):
    ...
    if rubber_dia:
        ...
        # Add locking ring cost for impact rollers (use pipe code, e.g., 88.9 -> 89)
        pipe_code = get_pipe_code(pipe_dia)
        locking_ring_cost = price_loader.get_locking_ring_cost(pipe_code, LOCKING_RING_COSTS)
